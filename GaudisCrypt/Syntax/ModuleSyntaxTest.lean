...
#check (M2.g.mk_simp : ∀ s : M2.Structure, M2.g (M2.mk s) = s.g)
#check (M2.h.mk_simp : ∀ s : M2.Structure, M2.h (M2.mk s) = s.h)
example (a : Module.Arr TestModule (procmod () → Unit)) (b : TestModule) :
    M2.g (Module.app Y (Module.pair a b))
      = (Module.app (Module.app Y.g a) b : Module.Proc (procsig () -> Unit)) := by
      simp only [Y.apply_simp, M2.g.mk_simp]
-- it is `@[simp]`, so it also fires on its own
example (s : M2.Structure) : M2.h (M2.mk s) = s.h := by simp
...
-- `mk_simp` carries the parameters the same way, and still fires from its `@[simp]` alone
#check (Sized.gen.mk_simp :
  ∀ (n : Nat) (s : Sized.Structure n), Sized.gen n (Sized.mk n s) = s.gen)
